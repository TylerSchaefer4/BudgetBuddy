//
//  RightBarButtonHomeScreen.swift
//  BudgetBuddy
//
...
extension ViewController{
    ...
    func signInToFirebase(email: String, password: String) {
        ...
        Auth.auth().signIn(withEmail: email, password: password) { [weak self] (_, error) in
            if error == nil {
                // User authenticated
                self?.showAlert(title: "Success", message: "User authenticated")
            } else {
                // Alert that no user found or password is wrong
                self?.showAlert(title: "Error", message: "No user found or incorrect password")
            }
        }
    }
